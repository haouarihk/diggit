...
export function decodePullRequestSource(value?: string | null) {
  if (!value) {
    return null;
  }
...
export function sourceFromOption(
  option: PullRequestSourceOption,
  branch: string,
...
export function fallbackBranches(name: string) {
...
export function defaultBranch(branches: RepositoryBranch[], preferred: string) {
  return (
    branches.find((branch) => branch.name === preferred)?.name ??
...
export function remoteRepositoryGitUrl(
  serverName: string,
  owner: string,
  name: string,
) {
