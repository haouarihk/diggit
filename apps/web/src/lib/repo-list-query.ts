...
export function buildListHref(path: string, params: Record<string, number | string | undefined | null>) {
...
export function getIssueSearchInput(rawQuery?: string, legacy?: { labels?: string; status?: string }) {
  const normalized = rawQuery?.trim();
  if (normalized) {
    return normalized;
  }
  const labels = splitList(legacy?.labels);
  const status = legacy?.status?.trim() === "all" ? null : normalizeIssueStatus(legacy?.status) ?? "open";
  return serializeIssueLikeQuery({ labels, status, terms: [] });
}

export function parseIssueSearchQuery(rawQuery?: string, legacy?: { labels?: string; status?: string }): IssueSearchState {
...
export function setIssueSearchStatusQuery(rawQuery: string | undefined, status: "closed" | "open" | null, legacy?: { labels?: string; status?: string }) {
  const current = parseIssueLikeQuery(getIssueSearchInput(rawQuery, legacy), ["open", "closed"]);
  return serializeIssueLikeQuery({ ...current, status });
}

export function toggleIssueSearchLabelQuery(rawQuery: string | undefined, label: string, legacy?: { labels?: string; status?: string }) {
  const current = parseIssueLikeQuery(getIssueSearchInput(rawQuery, legacy), ["open", "closed"]);
  return serializeIssueLikeQuery({ ...current, labels: toggleValue(current.labels, label) });
}
...
export function parsePullRequestSearchQuery(rawQuery?: string): PullRequestSearchState {
...
export function setPullRequestSearchStatusQuery(rawQuery: string | undefined, status: "closed" | "merged" | "open" | null) {
  const current = parseIssueLikeQuery(getPullRequestSearchInput(rawQuery), ["open", "closed", "merged"]);
  return serializeIssueLikeQuery({ ...current, status });
}

export function togglePullRequestSearchLabelQuery(rawQuery: string | undefined, label: string) {
  const current = parseIssueLikeQuery(getPullRequestSearchInput(rawQuery), ["open", "closed", "merged"]);
  return serializeIssueLikeQuery({ ...current, labels: toggleValue(current.labels, label) });
}
...
export function parseReleaseSearchQuery(rawQuery?: string): ReleaseSearchState {
...
export function toggleReleasePrereleaseQuery(rawQuery: string | undefined) {
  ...
  return serializeReleaseSearchQuery({ ...current, isPrerelease: !current.isPrerelease });
}

export function toggleReleaseTagQuery(rawQuery: string | undefined, tag: string) {
  const current = parseReleaseSearchQuery(rawQuery);
...
function parseIssueLikeQuery<TStatus extends IssueLikeStatus>(
  ...
  for (const token of tokenizeQuery(query)) {
    ...
    if (qualifier.key === "is") {
      const normalizedStatus = normalizeIssueLikeStatus(qualifier.value, supportedStatuses);
      if (normalizedStatus) {
...
function serializeIssueLikeQuery<TStatus extends IssueLikeStatus>(state: IssueLikeParsedState<TStatus>) {
  const tokens: string[] = [];
  if (state.status) {
...
function normalizeIssueLikeStatus<TStatus extends IssueLikeStatus>(value: string | undefined, supportedStatuses: readonly TStatus[]) {
  if (!value) {
    return null;
  }
...
function uniqueValues(values: string[]) {
  return values.filter((value, index) => values.findIndex((entry) => sameValue(entry, value)) === index);
}
