import { getRepositoryFile, type RepositoryFile, type RepositoryTreeEntry } from "@/lib/api";

export function findReadmeEntry(entries: RepositoryTreeEntry[]) {
  return entries.find((entry) => entry.kind === "file" && entry.name.toLowerCase() === "readme.md") ?? null;
}
